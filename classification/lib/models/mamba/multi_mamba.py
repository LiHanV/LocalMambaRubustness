...
try:
    ...
    mamba_inner_fn = None
    print("Warning: mamba_inner_fn not found, please check mamba-ssm installation.")
...
class MultiMamba(nn.Module):
    def __init__(
        ...
        super().__init__()
        print(f"MultiMamba directions: {directions}")
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        # print(f"Layer {self.layer_idx}: xs length = {len(xs)}")
        if len(xs) == 0:
            print(f"ERROR: xs is empty! xz.shape = {xz.shape}, token_size = {self.multi_scan.token_size}")
        ...
        for i, xz in enumerate(xs):
            ...
            # 为每个方向创建单位矩阵和零偏置
            out_proj_weight = torch.eye(self.d_inner, device=xz.device, dtype=xz.dtype)
            out_proj_bias = torch.zeros(self.d_inner, device=xz.device, dtype=xz.dtype)
            ...
            out = mamba_inner_fn(
                ...
                dt_proj.weight,             # delta_proj_weight
                out_proj_weight,            # out_proj_weight (单位矩阵)
                out_proj_bias,              # out_proj_bias (零偏置)
                A,                          # A
                None,                       # B (input-dependent)
                None,                       # C (input-dependent)
                D,                          # D
                dt_proj.bias.float(),       # delta_bias
                None,                       # B_proj_bias
                None,                       # C_proj_bias
                True,                       # delta_softplus
            )
            # print(f"Layer {self.layer_idx}, direction {i}: out shape = {out.shape if out is not None else None}")
            out = rearrange(out, 'b l d -> b d l')   # 转换为 (B, D, L)
            outs.append(out)
        
        # print(f"Layer {self.layer_idx}: after loop, len(outs) = {len(outs)}")
        ...
        # print(f"Layer {self.layer_idx}: after multi_reverse, len(outs) = {len(outs)}")
        ...
        # print(f"Layer {self.layer_idx}: after attn, len(outs) = {len(outs)}")
        out = self.multi_scan(outs)
        # print(f"Layer {self.layer_idx}: before multi_scan, len(outs) = {len(outs)}")  # 如果出错，此行不会执行
